...
def generate_pdf_report():
    ...
    pdf.set_xy(10, 7)
    pdf.cell(0, 10, "NTPC IT Division - Maintenance & Warranty Report", ln=True)
    pdf.set_font("Helvetica", "", 10)
    pdf.set_xy(10, 20)
    pdf.cell(0, 6, sanitize(f"Generated: {datetime.now().strftime('%d %B %Y %H:%M')}  |  Total Assets: {len(df)}"), ln=True)
    pdf.ln(15)
